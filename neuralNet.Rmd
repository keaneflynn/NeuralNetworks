...
sum((groundtruth_data - predicted_values)^2)/length(groundtruth_data)
```
...
plot(groundtruth_data, predicted_values, col='red', pch=1, 
     ...
     main = "NHSE Custody Period Prediction")
abline(a=0,b=1) #This b=1 line shows what the data would look like in an idea scenario
sum((groundtruth_data - predicted_values)^2)/length(groundtruth_data) #arbitrary calculation for difference between actual and predicted value
```
...
Instead of chopping data on the tail end like we did, we can thin the data across the range of the training dataset to better acheive this uniform distribution. 
...
Like I mentioned earlier, these models only get better the more data that is collected. As ecological research scales with modern remote sensing and large scale data collection, neural networks are going to become increasingly prevalent in both data collection as well as interpretation. 
